...
class TRGAT(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.Implict_GAT = GAttNet(1, 1, self.vfeat_size, self.vfeat_size)
        self.Spatial_GAT = GAttNet(2, 11, self.vfeat_size, self.vfeat_size)
    ...
    def forward_main(self, batch):
        ...
        if self.vfeat_flag:
            ...
            # vid_embed = vid_embed.contiguous().view(bsz, num_imgs, num_regions, -1)  # (N, 1, Li, Lr, D)
            for idx in range(vid_embed.shape[1]):
                ...
                adj_matrix = batch["imp_adj_mtx"][:, idx]
                img_embed = self.Implict_GAT(vid_embed[:, idx], adj_matrix)
                if idx == 0:
                    vid_embed_graph = img_embed.unsqueeze(1)
                else:
                    vid_embed_graph = torch.cat((vid_embed_graph, img_embed.unsqueeze(1)), dim = 1)
            
            # vid_embed_graph = vid_embed_graph.contiguous().view(bsz* num_imgs, num_regions, -1)  # (N, Li, Lr, D)
            # Used when GAT before encoder
            vid_embed_graph = vid_embed_graph.contiguous().view(bsz, num_imgs, num_regions, -1)  # (N, Li, Lr, D)        

            # Residual connection
            vid_embed = vid_embed + vid_embed_graph    
        ...
        if self.vcpt_flag:
            ...
            for idx in range(vcpt_embed.shape[1]):
                adj_matrix = batch["spa_adj_mtx"][:, idx]
                img_embed = self.Spatial_GAT(vcpt_embed[:, idx], adj_matrix)
                if idx == 0:
                    vcpt_embed_graph = img_embed.unsqueeze(1)
                else:
                    vcpt_embed_graph = torch.cat((vcpt_embed_graph, img_embed.unsqueeze(1)), dim = 1)

            # vcpt_embed_graph = vcpt_embed_graph.contiguous().view(bsz* num_imgs, num_regions, -1)  # (N, Li, Lr, D)
            # Used when GAT before encoder
            vcpt_embed_graph = vcpt_embed_graph.contiguous().view(bsz, num_imgs, num_regions, -1)  # (N, Li, Lr, D)            
            # Residual connection
            vcpt_embed = vcpt_embed + vcpt_embed_graph
        ...
        if self.flag_cnt == 2:
            ...
            # visual_text_embedding = torch.stack([attended_sub, attended_vid, attended_sub * attended_vid], dim = -2)
            ...
            visual_text_embedding_score = F.softmax(self.MultimodelAttention(visual_text_embedding), dim = -2) * 3
            visual_text_embedding_score_trans = visual_text_embedding_score.transpose(-1, -2)
            visual_text_embedding_score = torch.matmul(visual_text_embedding_score, visual_text_embedding_score_trans)
            visual_text_embedding = torch.matmul(visual_text_embedding_score, visual_text_embedding)
        ...
        return out, target, F.softmax(t_scores, dim=2), vid_raw_s, temporal_loss, att_loss
    pass
    ...
    def advanced_encoder(self, inputs):
        ...
        self_weight_mask = torch.zeros(batch_size,img_num).to(self.opt.device)
    ...
    def get_att_loss(self, scores, att_labels, target, words, vid_names, qids, q_lens, img_indices, boxes,
        ...
        for batch_idx in range(len(target)):  # batch
            ...
            for img_idx, img_gt_score in enumerate(gt_score):
                ...
                img_pos_indices = torch.nonzero(img_gt_score, as_tuple = False)  # (N_pos, 2) ==> (#words, #regions)
                ...
                img_neg_indices = torch.nonzero(img_gt_score == 0, as_tuple = False)  # (N_neg, 2)
